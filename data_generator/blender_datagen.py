...
sys.path.append(
    LIB_PATH + "blender-4.0.2-linux-x64/blender_env/lib/python3.10/site-packages"
)
...
class SceneGenerator:
    def __init__(self) -> None:
        self.description = (
            "This class generates a scene with a floor plane and a camera"
        )
    ...
    def create_floor(self, location=(0, 0, 0), scale=(1, 1, 1)):
        bpy.ops.mesh.primitive_plane_add(
            enter_editmode=False, align="WORLD", location=location, scale=scale
        )
    ...
    def create_multiple_floors(self, texture_paths, locations_list, scale_list):
        for texture_path, location, scale in zip(
            texture_paths, locations_list, scale_list
        ):
            # Create floor
    ...
    def get_calib_matrix(self):
        ...
        self.calib_matrix = [
            [self.cam_focal_length, 0, self.principal_point[0]],
            [0, self.cam_focal_length, self.principal_point[1]],
            [0, 0, 1],
        ]
    ...
    def setup_camera_with_intrinsics(self):
        ...
        bpy.ops.object.camera_add(
            enter_editmode=False, align="VIEW", location=(0, 0, 5), rotation=(0, 0, 0)
        )
        ...
        bpy.context.object.data.angle_x = 2 * math.atan(
            calibration_matrix[0][2] / calibration_matrix[0][0]
        )  # Horizontal FOV
        bpy.context.object.data.angle_y = 2 * math.atan(
            calibration_matrix[1][2] / calibration_matrix[1][1]
        )  # Vertical FOV

        # Set principal point
        bpy.context.object.data.shift_x = (
            calibration_matrix[0][2] - bpy.context.object.data.sensor_width / 2
        ) / bpy.context.object.data.sensor_width
        bpy.context.object.data.shift_y = (
            calibration_matrix[1][2] - bpy.context.object.data.sensor_height / 2
        ) / bpy.context.object.data.sensor_height
    ...
    def render_and_save(self, render_path, render_rgb_path):
        ...
        bpy.context.scene.render.engine = "BLENDER_EEVEE"
        bpy.context.scene.render.resolution_x = self.image_height
        bpy.context.scene.render.resolution_y = self.image_width
        bpy.context.scene.render.image_settings.file_format = "PNG"
...
temp_texture_paths = "/home/udaygirish/Projects/WPI/computer_vision/project4/p2/Visual-Inertial-Odometry/textures/Texture1.png"  # aerial_rocks_02_diff_4k.jpg"
...
scenegen.create_grid_of_floors(texture_paths, grid_size=(20, 20), scale=(10, 10, 1))
...
for i in range(trajectory_time):
    ...
    # Generate IMU data
    t_array, real_acc, real_gyro = generate_imu_data(
        dx,
        dy,
        dz,
        dyaw,
    ...
    temp_render_rgb_path = os.path.join(
        RENDER_BASE_PATH, "image_rgb_{}.png".format(i + 1)
    )
    scenegen.render_and_save(temp_render_path, temp_render_rgb_path)
