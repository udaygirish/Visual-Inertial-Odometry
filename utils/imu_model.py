...
def vib_from_env(env, fs):
    ...
    if env is None:
        ...
        vib_amp = (
            1.0  # vibration amplitude, 1sigma for random, peak value for sinusoidal
        )
...
def bias_drift(corr_time, drift, n, fs):
    ...
    for i in range(0, 3):
        if not math.isinf(corr_time[i]):
            ...
            for j in range(1, n):
                sensor_bias_drift[j, i] = (
                    a * sensor_bias_drift[j - 1, i] + b * drift_noise[j - 1, i]
                )
        else:
            # normal distribution
...
def acc_gen(fs, ref_a, acc_err, vib_def=None):
    ...
    if vib_def is not None:
        if vib_def["type"].lower() == "psd":
            acc_vib[:, 0] = time_series_from_psd(vib_def["x"], vib_def["freq"], fs, n)[
                1
            ]
            acc_vib[:, 1] = time_series_from_psd(vib_def["y"], vib_def["freq"], fs, n)[
                1
            ]
            acc_vib[:, 2] = time_series_from_psd(vib_def["z"], vib_def["freq"], fs, n)[
                1
            ]
            ...
            acc_vib[:, 0] = vib_def["x"] * np.sin(
                2.0 * math.pi * vib_def["freq"] * dt * np.arange(n)
            )
            acc_vib[:, 1] = vib_def["y"] * np.sin(
                2.0 * math.pi * vib_def["freq"] * dt * np.arange(n)
            )
            acc_vib[:, 2] = vib_def["z"] * np.sin(
                2.0 * math.pi * vib_def["freq"] * dt * np.arange(n)
            )
...
def gyro_gen(fs, ref_w, gyro_err, vib_def=None):
    ...
    if vib_def is not None:
        if vib_def["type"].lower() == "psd":
            gyro_vib[:, 0] = time_series_from_psd(vib_def["x"], vib_def["freq"], fs, n)[
                1
            ]
            gyro_vib[:, 1] = time_series_from_psd(vib_def["y"], vib_def["freq"], fs, n)[
                1
            ]
            gyro_vib[:, 2] = time_series_from_psd(vib_def["z"], vib_def["freq"], fs, n)[
                1
            ]
        elif vib_def["type"] == "random":
            ...
            gyro_vib[:, 0] = vib_def["x"] * np.sin(
                2.0 * math.pi * vib_def["freq"] * dt * np.arange(n)
            )
            gyro_vib[:, 1] = vib_def["y"] * np.sin(
                2.0 * math.pi * vib_def["freq"] * dt * np.arange(n)
            )
            gyro_vib[:, 2] = vib_def["z"] * np.sin(
                2.0 * math.pi * vib_def["freq"] * dt * np.arange(n)
            )
